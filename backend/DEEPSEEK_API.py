...
# ===================== 配置项 =====================
# DeepSeek API配置（优先从环境变量读取，也可直接赋值）
DEEPSEEK_API_KEY = os.environ.get('DEEPSEEK_API_KEY', "sk-19d3a7b8976a4851bb36649fd07f084c")
DEEPSEEK_BASE_URL = "https://api.deepseek.com"

# 文件路径配置
INPUT_JSON_PATH = "crawled_data1.json"  # 原始非结构化JSON数据
OUTPUT_JSON_PATH = "formatted_robot_data3.json"  # 大模型输出的标准化JSON
...
### 输出格式要求（必须严格遵守，缺失属性填充"无"）：
{
  "nodes": [
    {
      "label": "Robot/Company/Technology/Application/Component",
      "properties": {
        ...
        // Technology属性：name、category（技术分类）、description（技术核心特性/功能，提取所有相关描述，整合为通顺、逻辑清晰的一段话）
        // Application属性：name、domain（场景领域）
        // Component属性：name、type（部件类型）、specifications（部件规格参数，提取所有相关参数，保留单位，用分号分隔）
        // 所有缺失的属性值必须填充为"无"
      }
    }
  ],
  "relations": [
    {
      "type": "DEVELOPED_BY/USES_TECH/CONTAINS/APPLIED_IN",
      "start_node_label": "起始节点类型（如Robot）",
      "start_node_name": "起始节点名称",
      "end_node_label": "终止节点类型（如Company）",
      "end_node_name": "终止节点名称",
      "properties": {
        // DEVELOPED_BY（机器人由某企业研发）属性：关联方向：Robot → Company；year（研发/发布年份，无则填"无"）、version（产品版本，缺失填"无"）
        // USES_TECH（机器人采用某核心技术）属性：关联方向：Robot → Technology；空字典{}
        // CONTAINS（机器人包含某核心部件）属性：关联方向：Robot → Component；quantity（部件数量/配置，缺失填"无"）
...
2. 节点label只能是Robot/Company/Technology/Application/Component中的一种
3. 关系type只能是DEVELOPED_BY/USES_TECH/CONTAINS/APPLIED_IN中的一种
...
def auto_fill_missing_properties(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    node_prop_templates = {
        ...
        "Component": ["name", "type", "specifications"]
    }

    # 关系属性模板
    rel_prop_templates = {
        "DEVELOPED_BY": ["year", "version"],
        "USES_TECH": [],
        "CONTAINS": ["quantity"],
