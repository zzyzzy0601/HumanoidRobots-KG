...
class MultiRobotSpider:
    ...
    def clean_text(self, text_list):
        """清洗提取到的文本列表"""
        noise_keywords = ['版权', '联系我们', '电话', '菜单', '微信', 'Copyright', '©', 'App下载']
        cleaned = []
        for t in text_list:
            t = t.strip()
    ...
    def run(self):
        # 1. 宇树科技
        for url in ["https://www.unitree.com/cn/h1", "https://www.unitree.com/cn/g1","https://www.unitree.com/cn/R1","https://www.unitree.com/cn/H2"]:
            self.parse_unitree(url)
            time.sleep(1)

        # 2. 松延动力
        for url in ["https://noetixrobotics.com/n2", "https://noetixrobotics.com/e1"]:
            self.parse_others("松延动力", url)
            time.sleep(1)

        # 3. 魔法原子
        for url in ["https://www.magiclab.top/z1", "https://www.magiclab.top/human"]:
